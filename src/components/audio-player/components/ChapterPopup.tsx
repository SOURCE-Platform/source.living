...
export const ChapterPopup: React.FC<ChapterPopupProps> = ({ title, chapters, onChapterClick, onClose }) => {
    ...
    return (
        <div
            className="absolute bottom-[calc(100%+38px)] left-[-16px] w-[280px] bg-[#F4F4F5] dark:bg-[#1F1F28] rounded-2xl border border-black/5 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.24)] overflow-hidden flex flex-col max-h-[300px] z-[90] animate-in slide-in-from-bottom-2 fade-in duration-200 origin-bottom-left"
        >
            ...
            <div className="overflow-y-auto py-1 scrollbar-thin scrollbar-thumb-black/10 dark:scrollbar-thumb-white/10 scrollbar-track-transparent">
                {chapters.map((chapter, i) => (
                    <button
                        ...
                        className="w-full text-left px-4 py-2.5 hover:bg-black/5 dark:hover:bg-white/5 transition-colors flex items-center gap-3 group border-l-2 border-transparent hover:border-black/20 dark:hover:border-white/20"
                    >
