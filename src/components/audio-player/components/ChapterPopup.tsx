            ...
            <div className="overflow-y-auto py-1 scrollbar-thin scrollbar-thumb-black/10 dark:scrollbar-thumb-white/10 scrollbar-track-transparent">
                {chapters.map((chapter, i) => (
                    <button
                        ...
                        className="w-full text-left px-4 py-2.5 hover:bg-black/5 dark:hover:bg-white/5 transition-colors flex items-start gap-3 group border-l-2 border-transparent hover:border-black/20 dark:hover:border-white/20 cursor-pointer"
                    >
                        <span className="text-xs font-mono text-muted-foreground group-hover:text-foreground transition-colors min-w-[35px] opacity-70 mt-0.5">
                            {formatTime(chapter.start)}
                        </span>
                        <span className="text-sm text-foreground flex-1 font-medium opacity-90 group-hover:opacity-100 leading-tight">
                            {chapter.title}
                        </span>
                    </button>
                ))}
