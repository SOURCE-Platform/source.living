...
// SmokeParticleMaterial moved to ./SmokeEffect.tsx


extend({ WarpShaderMaterial });

// Types for SmokeParticleMaterial interface moved to separate file or inferred. 
// If specific types for internal use are needed they should be imported or redeclared minimally if strictly necessary, 
// but since we are replacing the component, we might not need them here if we use the imported component.
...
const ImagePlane = ({ imageSrc, onLoad, mountTime }: { imageSrc: string, onLoad?: () => void, mountTime: number }) => {
    ...
    return (
        <group scale={[planeWidth, planeHeight, 1]}>
            <SmokeParticles materialRef={smokeMaterial} autoUpdate={false} />
